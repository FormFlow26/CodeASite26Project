...
export async function createSessionManager({
  ...
  async function startSession() {
    const { response, body } = await request(
      ...
      {
        method: "POST",
        body: JSON.stringify({ userId, groupId, exerciseType, targetSets, repsPerSet })
      },
      "session-init"
    );
  ...
  function buildRepPayload(repData) {
    ...
    return {
      ...
      flags,
      points,
      durationMs: roundToTwo(repData.durationMs),
      snapshots: Array.isArray(repData.snapshots) ? repData.snapshots : []
    };
  }
