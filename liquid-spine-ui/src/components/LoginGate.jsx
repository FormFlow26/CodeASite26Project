...
const LoginGate = ({ authMode = 'login', onLogin, onSwitchMode, onBack }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (authMode === 'signup' && !trimmedName) {
      setError('Enter your full name to create your FormFlow account.');
      return;
    }
        ...
        <form className="glass-panel login-card" onSubmit={handleSubmit}>
          <div className="login-card-header">
            ...
            <p className="card-copy">
              {authMode === 'signup'
                ? 'Use this as the sign-up entry point until the backend auth flow is wired.'
                : 'This screen is ready for real authentication wiring when the backend is connected.'}
            </p>
          </div>
          ...
          <button type="submit" className="primary-button login-submit">
            {authMode === 'signup' ? 'Create Account' : 'Enter FormFlow'}
          </button>
        </form>
      </div>
    </div>
